...
export default function PublicProfile({ profile, links }) {
  if (!profile) return (
    <div style={{ minHeight:'100vh', background:'#0f0f0f', display:'flex', alignItems:'center',
      justifyContent:'center', color:'#fff', fontFamily:'system-ui', textAlign:'center' }}>
...
export async function getServerSideProps({ params }) {
  ...
  const { data: links } = await supabase
    .from('links').select('*').eq('user_id', profile.id).eq('active', true).order('position')
